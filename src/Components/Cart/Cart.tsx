    ...
    const cartItems = cart.map((item: any, i: number) => <div className={s.cartItems} key={i}>
        <div className={s.cartItem}>
            <div className={s.close} onClick={() => {
                deleteFromCartById(item[0].id)
            }} style={{fontSize: "30px", margin: "auto 0 auto 0", marginRight: "20px"}}>&times;</div>
            <h4 style={{margin: "auto 0 auto 0", marginRight: "20px"}}>{i + 1}</h4>
            <img
                src={item[0].media.imageUrl ? item[0].media.imageUrl : "https://stockx-assets.imgix.net/media/New-Product-Placeholder-Default.jpg?fit=fill&bg=FFFFFF&w=700&h=500&auto=format,compress&trim=color&q=90&dpr=2&updated_at=0"}
                alt={"sneaker"}/>
            <h3 className={s.title}>{item[0].title}
                <br/> {item[0].retailPrice ? (item[0].retailPrice * rate).toFixed(0) + ` ${currentCurrency}` : "N/A"}
            </h3>
            <div className={s.input}>
                <button onClick={() => {
                    deleteItemC(item[0].id)
                ...
                <div style={{
                    ...
                    fontWeight: "bold",
                    fontSize: "25px"
                }}>{(total[i] * rate).toFixed(0)} {currentCurrency}</div>
            </div>
        </div>
    </div>)
    ...
    return (
        <div className={s.cart}>
            ...
            <div style={{textAlign: "center", fontSize: "20px"}}>
                <strong style={{paddingBottom: "10px", borderBottom: "1px solid"}}>Total
                    price: {total && ((total.reduce((accumulator: number, currentValue: number): number => {
                        return accumulator + currentValue
                    }, 0)) * rate).toFixed(0)} {currentCurrency}</strong>
