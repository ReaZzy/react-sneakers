...
export const Shoes = React.memo((props:any) => {
    ...
    const sneakersList = sneakers.results.map((sneaker:any, index:number) => {
        return <div style={{cursor:"pointer"}} className={s.card} key={index+1} id={sneaker.id}>
            <div  onClick={()=>{showMoreInfo(sneaker.id)}} className={sneaker.retailPrice ===0 || !sneaker.retailPrice? s.disabled: ""} >
                <img className={s.img} src={sneaker.media.imageUrl ? sneaker.media.imageUrl : "https://stockx-assets.imgix.net/media/New-Product-Placeholder-Default.jpg?fit=fill&bg=FFFFFF&w=700&h=500&auto=format,compress&trim=color&q=90&dpr=2&updated_at=0"} alt={"sneaker"}/>
            ...
            {sneaker.retailPrice !==0? <button onClick={()=>addToCartById(sneaker.id)} className={"btn btn-outline-warning"}>Cart</button> : ""}
    ...
    return (
        ...
        {sneakersList.length >= 1
                ?<><Paginator/><div style={{display:"flex", flexWrap:"wrap", justifyContent:"center"}}>
                    {sneakersList}
                </div><Paginator/></>
                :<div style={{display:"flex", flexWrap:"wrap", justifyContent:"center"}}>
                    {<h1 style={{margin:"0 auto", marginTop:"20px"}}>{`Can't find sneakers with name "${currentTerm}"`}</h1>}
                </div>
        }
            <Modal
